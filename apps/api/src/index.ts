...
if (env.appMode === "cloud") {
  const missing = [
    !env.databaseUrl && "DATABASE_URL", !(env.clerkSecretKey || env.clerkJwtKey) && "CLERK_SECRET_KEY",
    !env.stripeSecretKey && "STRIPE_SECRET_KEY", !env.stripeWebhookSecret && "STRIPE_WEBHOOK_SECRET", !env.stripePriceId && "STRIPE_PRICE_ID",
  ].filter(Boolean);
  if (missing.length) {
    console.error(`APP_MODE=cloud: es fehlen ${missing.join(", ")} (siehe .env.example).`);
    process.exit(1);
  }
} else if (env.isProduction && !env.dashboardPassword) {
...
if (env.appMode === "local" && env.dashboardPassword && env.dashboardPassword.length < minLength) {
