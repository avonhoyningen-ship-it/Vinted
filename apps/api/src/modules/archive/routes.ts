...
import { readPhoto, storePhoto } from "../../storage/photos.js";
...
archiveRouter.get("/:id/photos.zip", h(async (req, res) => {
  ...
  const files = await Promise.all(photos.map(async (p, i) => ({ name: `${String(i + 1).padStart(2, "0")}.jpg`, data: await readPhoto(p.file_name) })));
