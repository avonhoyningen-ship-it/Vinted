...
export function recordPriceExample(itemId: number, priceCents: number, source: ExampleSource) {
  const item = db.prepare("SELECT title, brand, category, size, condition FROM items WHERE id = ?").get(itemId) as ItemFeatures | undefined;
  if (!item || !priceCents) return;
  // One example per item for the seller's own decisions (latest wins); sales are kept separately.
  if (source !== "sold") db.prepare("DELETE FROM price_examples WHERE item_id = ? AND source <> 'sold'").run(itemId);
  db.prepare(`INSERT INTO price_examples (item_id, source, title, brand, category, size, condition, price_cents, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    .run(itemId, source, item.title, item.brand, item.category, item.size, item.condition, priceCents, nowIso());
}

/** Sets the price of an item as seller-confirmed and learns from it. */
export function confirmPrice(itemId: number, priceCents: number, source: Exclude<ExampleSource, "sold">) {
  db.prepare("UPDATE items SET price_cents = ?, price_confirmed = 1, updated_at = ? WHERE id = ?").run(priceCents, nowIso(), itemId);
  recordPriceExample(itemId, priceCents, source);
}
...
export function similarExamples(item: ItemFeatures, limit = 8, minScore = 2.5) {
  const all = db.prepare("SELECT * FROM price_examples ORDER BY created_at DESC LIMIT 2000").all() as unknown as PriceExample[];
  return all
...
export function matchingRule(item: ItemFeatures): PriceRule | null {
  const rules = db.prepare("SELECT * FROM price_rules ORDER BY created_at DESC").all() as unknown as PriceRule[];
...
export function suggestPrice(item: ItemFeatures, aiPriceCents?: number | null): Suggestion | null {
  const rule = matchingRule(item);
  if (rule) return { priceCents: rule.price_cents, reason: `Deine Regel: ${describeRule(rule)}`, source: "rule" };

  const similar = similarExamples(item);
  if (similar.length) {
...
export function examplesForPrompt(item: ItemFeatures): string {
  let list = similarExamples(item, 8, 1);
  if (!list.length) {
    list = (db.prepare("SELECT * FROM price_examples ORDER BY created_at DESC LIMIT 8").all() as unknown as PriceExample[]).map((e) => ({ e, score: 0 }));
  }
...
export function refreshSuggestion(itemId: number, aiPriceCents?: number | null): Suggestion | null {
  const item = db.prepare("SELECT id, title, brand, category, size, condition, price_suggested_cents, price_suggestion_reason FROM items WHERE id = ?")
    .get(itemId) as (ItemFeatures & { price_suggested_cents: number | null; price_suggestion_reason: string | null }) | undefined;
  if (!item) return null;
  ...
  const s = suggestPrice(item, ai);
  db.prepare("UPDATE items SET price_suggested_cents = ?, price_suggestion_reason = ? WHERE id = ?").run(s?.priceCents ?? null, s?.reason ?? null, itemId);
  return s;
}

/** Recomputes suggestions for all items still waiting for a confirmed price. */
export function refreshAllSuggestions(): number {
  const ids = db.prepare("SELECT id FROM items WHERE price_confirmed = 0 AND status IN ('draft','archived')").all() as { id: number }[];
  for (const { id } of ids) refreshSuggestion(Number(id));
  return ids.length;
}
